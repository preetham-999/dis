...
namespace ISM6225_Fall_2023_Assignment_2
{
    class Program
    {
            ...
            try
            {
                ...
                foreach (int num in nums)
                {
                    if (num > range)
                    {
                        if (num -range == 1)
                        {
                            result.Add(new[] { (int)range, (int)range }.ToList());
                ...
                if (range <= upper)
                {
                    ...
                    {
                        int value = (int)range;
                        result.Add(new List<int> { value, value });

                    }
                    else if (upper - range > 0)
                    {
                        var pair = new List<int> { (int)range, (int)upper };
                        result.Add(pair);
     ...
     public static bool IsStrobogrammatic(string s1)
        {
           try
		   {
            ...
            while (left <= right)
            {
                if (!pairs.ContainsKey(s1[left]) || pairs[s1[left]] != s1[right])
                {
                    return false;
                }
        ...
        */
        ...
        public static int ThirdMax(int[] nums)
        {
            try
            {
                HashSet<int> max = new HashSet<int>();
